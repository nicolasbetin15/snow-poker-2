// pm2 process definition for frost-trio (FrostTrioBlend).
// SECURITY: no wallet/hotkey/port/repo hardcoded — everything comes from .env.
// (For the whole 12-miner fleet at once use /root/pokers/ecosystem.fleet.config.js.)
const fs = require("fs");
...
const CANDIDATES = [
  ...
  "/root/bittensor-venv/bin/python",
];
...
const NAME = E.POKER44_PM2_NAME || "poker44-frost-trio";
if (!WALLET || !HOTKEY) {
  throw new Error("frost-trio: missing POKER44_WALLET_NAME / POKER44_WALLET_HOTKEY in .env");
}
